...
import static willow.train.kuayue.Blocks.Bogeys.selection_menu.BogeyCategoryHandlerClient.MANAGE_FAVORITES_CATEGORY;
import static willow.train.kuayue.Blocks.Bogeys.selection_menu.BogeyCategoryHandlerClient.optimizeFavorites;

public class RadialBogeyCategoryMenu extends AbstractSimiScreen {

    private willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.State state;
    ...
    public RadialBogeyCategoryMenu(willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.State state) {
        this.state = state;
        hoveredSlot = -1;
    }
    ...
    private void renderInInventory(double posX, double posY, int scale, Consumer<willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.RenderInfo> render) {
        ...
        try {
            RenderSystem.runAsFancy(() -> render.accept(new willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.RenderInfo(ms, bufferSource, 0xF000F0)));
        } catch (Exception e) {
            if (Utils.isDevEnv())
                throw e;
        }
...
/*
        ...
        if (favoriteSlot == null && selectedCategory != MANAGE_FAVORITES_CATEGORY) {
            ...
            KYIcons.I_FAVORITE.render(ms, -9, -9, this);
            ms.translate(0.5, -0.5, 0);
        ...
        for (int slot = 0; slot < 8; slot++) {
            ...
            if (state == willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.State.PICK_CATEGORY) {
            ...
            } else if (state == willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.State.PICK_STYLE) {
                if (slot < BogeyCategoryHandlerClient.styleCount(selectedCategory)) {
                    ...
                    BogeySizes.BogeySize size = style.validSizes().toArray(BogeySizes.BogeySize[]::new)[sizeIdx % style.validSizes().size()];
                    ...
                    if (block instanceof AbstractBogeyBlock<?> bogeyBlock && minecraft != null) {
                        ...
                        Consumer<willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.RenderInfo> render = (info) -> {
    ...
    @Override
    public void tick() {
        if (state == willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.State.PICK_STYLE && selectedCategory == null)
            state = willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.State.PICK_CATEGORY;
        ticksOpen++;
        super.tick();
    }
    ...
    public boolean mouseClicked(double x, double y, int button) {
        ...
        if (button == 0) {
            if (selected == MANAGE_FAVORITES) {
                state = willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.State.PICK_STYLE;
                selectedCategory = MANAGE_FAVORITES_CATEGORY;
                return true;
            }
            if (state == willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.State.PICK_CATEGORY && selected >= 0 && selected < BogeyCategoryHandlerClient.categoryCount()) {
                state = willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.State.PICK_STYLE;
                selectedCategory = BogeyCategoryHandlerClient.getCategoryId(selected);
                return true;
            }

            if (state == willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.State.PICK_STYLE && selectedCategory == MANAGE_FAVORITES_CATEGORY) {
                state = willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.State.PICK_CATEGORY;
            ...
            if (state == willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.State.PICK_STYLE && selected >= 0 && selected < BogeyCategoryHandlerClient.styleCount(selectedCategory)) {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        ...
        if (distance <= 150) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                if (state == willow.train.kuayue.Blocks.Bogeys.selection_menu.RadialBogeyCategoryMenu.State.PICK_STYLE) {
