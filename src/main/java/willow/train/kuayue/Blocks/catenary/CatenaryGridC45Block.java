...
public class CatenaryGridC45Block extends HorizontalBlockBase implements CatenaryBlockInterface {

    Vec3[] CATENARYPORTS = new Vec3[]{};

    public CatenaryGridC45Block(Properties p_49795_) {
        super(p_49795_);
    }
    ...
    public Vec3 getCatenaryPort(BlockPos pPos, BlockState pState, Direction direction) {
        ...
        switch ((Direction) pState.getValue(FACING)) {
            case NORTH:
                if(yRot == 0 || yRot == 270){
                    return new Vec3(((double)pPos.getX())-0.5 - offset , pPos.getY() , (double)(pPos.getZ()) + 1.5 - offset);
                }
                return new Vec3(((double)pPos.getX())-0.5 + offset , pPos.getY() , (double)(pPos.getZ()) + 1.5 + offset);
            case SOUTH:
                if(yRot == 90 || yRot == 180){
                    return new Vec3((double)pPos.getX()+1.5 + offset , pPos.getY() ,  ((double)pPos.getZ())-0.5 + offset);
                }
                return new Vec3((double)pPos.getX()+1.5 - offset , pPos.getY() ,  ((double)pPos.getZ())-0.5 - offset);
            case WEST:
                if(yRot == 180 || yRot == 270){
                    return new Vec3(((double)pPos.getX()) + 1.5 - offset , pPos.getY() , (double) pPos.getZ() + 1.5 + offset);
                }
                return new Vec3(((double)pPos.getX()) + 1.5 + offset , pPos.getY() , (double) pPos.getZ() + 1.5 - offset);
            case EAST:
            default:
                if (yRot == 0 || yRot == 90){
                    return new Vec3(((double)pPos.getX()) - 0.5 + offset , pPos.getY() , (double) pPos.getZ() - 0.5 - offset);
                }
                return new Vec3(((double)pPos.getX()) - 0.5 - offset , pPos.getY() , (double) pPos.getZ() - 0.5 + offset);
        }
    }
