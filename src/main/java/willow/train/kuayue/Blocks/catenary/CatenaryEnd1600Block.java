...
public class CatenaryEnd1600Block extends HorizontalBlockBase implements CatenaryBlockInterface {

    public CatenaryEnd1600Block(Properties p_49795_) {
        super(p_49795_);
    }

    protected static final VoxelShape WEST_AABB = Shapes.or(Block.box(0, -16, 6, 16, 16, 10));
    protected static final VoxelShape SOUTH_AABB = Shapes.or(Block.box(6, -16, 0,10 , 16, 16));
    protected static final VoxelShape EAST_AABB = Shapes.or(Block.box(0, -16, 6, 16, 16, 10));
    protected static final VoxelShape NORTH_AABB = Shapes.or(Block.box(6, -16, 0, 10, 16, 16));
    ...
    public Vec3 getCatenaryPort(BlockPos pPos, BlockState pState, Direction direction) {
        switch ((Direction) pState.getValue(FACING)) {
            case NORTH:
                return new Vec3(((double)pPos.getX())+0.5  , pPos.getY() , (double)(pPos.getZ())+1.9);
            case SOUTH:
                return new Vec3((double)pPos.getX()+0.5  , pPos.getY() ,  ((double)pPos.getZ())-0.9);
            case WEST:
                return new Vec3(((double)pPos.getX()) + 1.9 , pPos.getY() , (double) pPos.getZ() + 0.5);
            case EAST:
            default:
                return new Vec3(((double)pPos.getX()) - 0.9 , pPos.getY() , (double) pPos.getZ() + 0.5);
        }
    }
