...
public class CatenaryGridA145Block extends HorizontalBlockBase implements CatenaryBlockInterface {

    Vec3[] CATENARYPORTS = new Vec3[]{};

    public CatenaryGridA145Block(Properties p_49795_) {
        super(p_49795_);
    }
    ...
    public Vec3 getCatenaryPort(BlockPos pPos,BlockState pState, Direction direction) {
        switch ((Direction) pState.getValue(FACING)) {
            case NORTH:
                return new Vec3(((double)pPos.getX())-0.5 - 0.13 , pPos.getY() , (double)(pPos.getZ()) + 1.5 + 0.13);
            case SOUTH:
                return new Vec3((double)pPos.getX()+1.5 + 0.13 , pPos.getY() ,  ((double)pPos.getZ())-0.5 - 0.13);
            case WEST:
                return new Vec3(((double)pPos.getX()) + 1.5 + 0.13 , pPos.getY() , (double) pPos.getZ() + 1.5 + 0.13);
            case EAST:
            default:
                return new Vec3(((double)pPos.getX()) - 0.5 - 0.13 , pPos.getY() , (double) pPos.getZ() - 0.5 - 0.13);
        }
    }
