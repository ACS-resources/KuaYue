...
import willow.train.kuayue.Util.TextUtils;
...
import static willow.train.kuayue.Main.registrate;
import static willow.train.kuayue.compat.tracks.TrackCompatUtils.buildCompatModels;
import static willow.train.kuayue.compat.tracks.TrackCompatUtils.makeTrack;
import static willow.train.kuayue.init.KYItems.ITEM_INCOMPLETE_TRACK;
        ...
        for (String name : names) {
            ...
            if (baseBlock.isEmpty()) {
                ...
                if (isDataGen() || Utils.isDevEnv())
                    Main.LOGGER.error("Failed to locate base block at "+getSlabLocation(name)+" for "+asResource(name));
            }
